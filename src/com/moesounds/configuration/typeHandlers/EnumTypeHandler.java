...
/**
 * MyBatis type handler to handle mapping Java enums to their proper database fields. This type
 * handler does not have to contain a clause for every enum type, but only for enum types where the
 * value in the database column is not identical to the name of the enum constant.
 */
...
public class EnumTypeHandler<E extends MappedEnum> extends BaseTypeHandler<E> {
    ...
    /**
     * Creates a new handler for a specific class of enum. This will be invoked by MyBatis when
     * attempting to handle an enum it hasn't seen before.
     * 
     * @param type
     *            The class of the enum this type handler will handle.
     */
